...
```{r}
#| label: contiguous-censored-calendar
bank_calls |> 
  filter(DateTime < ymd_hms("2003-03-15 00:00:00")) |>
  mutate(
    DateTime = seq_along(DateTime)
  ) |> 
  ggplot(aes(x = DateTime, y = Calls)) +
  geom_line() +
  geom_vline(xintercept = (0:10)*169, linetype = "dashed", color = "gray50")
```
...
```{r}
#| label: ggtime-ggdist-calendar
ggplot(data = tibble::tibble(
  time = seq(as.POSIXct("2025-01-01"), by = "hour", length.out = 24*30), 
  dist = distributional::dist_normal(mean = sin((18 + seq_along(time))*pi/12), sd = 1)
)) + 
  ggdist::stat_ribbon(aes(x = time, ydist = dist)) + 
  scale_fill_brewer() + 
  ggtime::coord_calendar(time_rows = "1 week")
```
